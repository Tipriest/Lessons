...
inline Mat3f RotX(const float& roll){
    Eigen::AngleAxisf rot_mat(roll, Vec3f::UnitX());
    return rot_mat.matrix();
}

inline Mat3f RotY(const float& pitch){
    Eigen::AngleAxisf rot_mat(pitch, Vec3f::UnitY());
    return rot_mat.matrix();
}

inline Mat3f RotZ(const float& yaw){
    Eigen::AngleAxisf rot_mat(yaw, Vec3f::UnitZ());
    return rot_mat.matrix();
}
