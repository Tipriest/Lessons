...
def parse_args():
    ...
    # Dataset
    parser.add_argument(
        "--root", default="/home/tipriest/data/", help="data root"
    )
    parser.add_argument(
        "-d",
        "--dataset",
    ...
    parser.add_argument(
        "--ema", action="store_true", default=False, help="Model EMA"
    )
    parser.add_argument(
        "--min_box_size",
        default=8.0,
...
def train():
    ...
    if distributed_utils.is_main_process:
        ...
        model_copy.eval()
        compute_flops(model=model_copy, img_size=args.img_size, device=device)
        del model_copy
    if args.distributed:
        dist.barrier()
