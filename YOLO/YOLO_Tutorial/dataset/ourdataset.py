...
try:
    ...
    print("It seems that the COCOAPI is not installed.-2")

try:
    from .data_augment.yolov5_augment import (
...
class OurDataset(Dataset):
    ...
    def __init__(
        ...
        self.coco = COCO(
            os.path.join(
                self.data_dir, image_set, "annotations", self.json_file
            )
        )
    ...
    def load_mosaic(self, index):
        ...
        index_list = (
            np.arange(index).tolist()
            + np.arange(index + 1, len(self.ids)).tolist()
        )
        id1 = index
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--root",
        default="/Users/liuhaoran/Desktop/python_work/object-detection/dataset/AnimalDataset/",
        help="data root",
    )
    ...
    parser.add_argument(
        "--mixup", default=None, type=float, help="mixup augmentation."
    )
    parser.add_argument(
        "--is_train",
        action="store_true",
    ...
    transform, trans_cfg = build_transform(
        args, trans_config, 32, args.is_train
    )
