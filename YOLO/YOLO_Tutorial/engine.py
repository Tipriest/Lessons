...
class Yolov8Trainer(object):
    def __init__(
        ...
        self.optimizer_dict["weight_decay"] *= (
            self.args.batch_size * accumulate / 64
        )
        ## 构建YOLO项目的优化器
        ...
        if self.args.ema and distributed_utils.get_rank() in [-1, 0]:
            ...
            self.model_ema = ModelEMA(
                self.ema_dict, model, self.start_epoch * len(self.train_loader)
            )
        else:
            self.model_ema = None
    ...
    def train_one_epoch(self, model):
        ...
        for iter_i, (images, targets) in tqdm(
            ...
            if distributed_utils.is_main_process() and iter_i % 10 == 0:
                ...
                wandb.log(
                    {"Epoch": self.epoch + 1, "Iter": iter_i, "lr": cur_lr[2]}
                )
                # 打印模型的loss
    ...
    def check_second_stage(self):
        ...
        if (
            "degrees" in self.trans_cfg.keys()
            and self.trans_cfg["degrees"] > 0.0
        ):
        ...
        # 如果使用到了平移和缩放，也将其关闭
        if (
            "translate" in self.trans_cfg.keys()
            and self.trans_cfg["translate"] > 0.0
        ):
    ...
    def rescale_image_targets(
        ...
        new_img_size = random.randrange(
            old_img_size * multi_scale_range[0],
            old_img_size * multi_scale_range[1] + max_stride,
        )
...
class YoloxTrainer(object):
    def __init__(
        ...
        self.lr_scheduler, self.lf = build_lr_scheduler(
            self.lr_schedule_dict,
            self.optimizer,
            self.args.max_epoch - self.no_aug_epoch,
        )
        self.lr_scheduler.last_epoch = self.start_epoch - 1  # do not move
        if self.args.resume:
            self.lr_scheduler.step()
    ...
    def train(self, model):
        for epoch in range(self.start_epoch, self.args.max_epoch):
            ...
            # 检查当前是否进入训练的第二阶段，在第二阶段中，会关闭Mosaic & Mixup增强
            if (
                epoch >= (self.args.max_epoch - self.second_stage_epoch - 1)
                and not self.second_stage
            ):
                self.check_second_stage()
                # 保存第一阶段的最后一次的模型参数
            ...
            if (
                epoch >= (self.args.max_epoch - self.third_stage_epoch - 1)
                and not self.third_stage
            ):
                self.check_third_stage()
                # 保存第二阶段的最后一次的模型参数
                weight_name = "{}_last_weak_augment_epoch.pth".format(
                    self.args.model
                )
                checkpoint_path = os.path.join(self.path_to_save, weight_name)
                print(
                    "Saving state of the last weak augment epoch-{}.".format(
                        self.epoch + 1
                    )
                )
                torch.save(
