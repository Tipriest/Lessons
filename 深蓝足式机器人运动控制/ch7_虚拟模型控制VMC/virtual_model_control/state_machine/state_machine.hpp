...
public:
    StateMachine(RobotType robot_type, RemoteCommandType cmd_type=RemoteCommandType::kKeyBoard){
        const std::string activation_key = "~/raisim/activation.raisim";
        std::string urdf_path = "";
        if(cmd_type==RemoteCommandType::kKeyBoard) uc_ptr_ = std::make_shared<KeyboardInterface>();
        else if(cmd_type==RemoteCommandType::kRetroid) uc_ptr_ = std::make_shared<RetroidGamepadInterface>(12121);
        else if(cmd_type==RemoteCommandType::kSkydroid) uc_ptr_ = std::make_shared<SkydroidGamepadInterface>(12121);
        else uc_ptr_ = std::make_shared<KeyboardInterface>();
        ...
        sitdown_controller_ = std::make_shared<SitDownState>(robot_type, "sit_down", data_ptr);
