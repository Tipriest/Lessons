...
static constexpr int m = 2;  // input a delta->角加速度
...
class MpcCar {
 private:
  ...
  void linearization(const double& phi,//phi是角度，delta是角加速度
                     const double& v,
                     const double& delta) {
  ...
  int solveQP(const VectorX& x0_observe) {
    ...
    for (int i = 0; i < N_; ++i) {
      ...
      if (i == 0) {
        ...
        AA.block(i*n, 0, n, n)     = matrix_power(Ad, i+1);
