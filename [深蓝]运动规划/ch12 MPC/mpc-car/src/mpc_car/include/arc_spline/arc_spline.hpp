...
// band matrix solver
class band_matrix {
  // 这个类实现了带状矩阵（band matrix）的操作，带状矩阵是一种稀疏矩阵，其中大部分元素为零，
  // 只有靠近主对角线的若干个带状区域有非零元素。带状矩阵通常用于存储和求解具有稀疏性质的线性方程组。
 private:
 ...
 public:
  ...
  void lu_decompose();  //LU分解
  std::vector<double> r_solve(const std::vector<double>& b) const;
  std::vector<double> l_solve(const std::vector<double>& b) const;
  std::vector<double> lu_solve(const std::vector<double>& b,
                               bool is_lu_decomposed = false);
};
// spline类用于处理样条插值，特别是三次样条插值。
// 三次样条插值可以通过一组离散的点（x, y）来拟合一个平滑的函数曲线。
// 三次样条的特点是每一段区间内的函数是三次多项式，并且在每个区间的两端保持连续的一阶和二阶导数。
class spline {
 public:
  enum bd_type {
    first_deriv = 1,
...
/**
 * @brief ArcSpline类在spline类的基础上进行了扩展，
 * 特别用于计算具有弧长参数化的样条曲线（Arc-Length Parametrization）。
 * 该类的目的是根据给定的一组路径点生成样条，并提供基于弧长的查询接口。
 */
class ArcSpline{
 private:
  double arcL_;              // 路径的总弧长
  spline xs_, ys_;           // x和y方向的样条函数对象
  std::vector<double> sL_;   // 弧长列表
  std::vector<double> xL_;   // x坐标列表
  std::vector<double> yL_;   // y坐标列表

 public:
  ArcSpline(){};
  ~ArcSpline(){};
  void setWayPoints(const std::vector<double>& x_waypoints,
                    const std::vector<double>& y_waypoints);//根据输入的x和y路径点生成样条，并计算路径的弧长。
  Eigen::Vector2d operator()(double s, int n = 0);          // 根据弧长s获取对应位置的点，n表示求解该点的导数阶数。
  double findS(const Eigen::Vector2d& p);                   //给定一个二维点，返回其对应的弧长s
  inline double arcL() {
    return arcL_;
  };
};
